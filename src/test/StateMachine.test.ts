...
import { Consumer } from "@jonloucks/concurrency-ts/api/Types";
...
describe('StateMachine Suite', () => {
  ...
  function createStateMachine<T>(config: Config<T>): StateMachine<T> {
    return stateMachineFactory.createStateMachine<T>(config);
  }
