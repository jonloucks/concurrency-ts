...
import { AutoClose, typeToAutoClose } from "@jonloucks/contracts-ts/api/AutoClose";
import { create as createIdempotent, typeToOpen } from "../impl/Idempotent.impl";
import { assertGuard, mockGuardFix } from "./helper.test";
import { Open } from "@jonloucks/contracts-ts/api/Open";
import { guardFunctions } from "../api/Types";
import { guard as guardOpen } from "@jonloucks/contracts-ts/api/Open";
...
describe('Idempotent Guard Tests', () => {
  it('isIdempotent should return true for Idempotent', () => {
    const idempotent: MockProxy<Idempotent> = mock<Idempotent>();
    mockGuardFix(idempotent, 'getState', 'open');
    ok(isIdempotent(idempotent), 'Idempotent should return true');
  });
});

assertGuard(isIdempotent, 'getState');

describe('Idempotent Creation Tests', () => {
  //   it('should create Idempotent with Open interface', () => {
  //     const mockOpen = mock<{ open: () => AutoClose }>();
  //     mockOpen.open.mockReturnValue(mock<AutoClose>());
  ...
  //     const mockAutoOpen = mock<{ autoOpen: () => AutoClose }>();
  //     mockAutoOpen.autoOpen.mockReturnValue(mock<AutoClose>());
  ...
  //     const mockAutoClose = mock<AutoClose>();
  //     const openFn = () => mockAutoClose;
  ...
  it('should transition state from OPENABLE to OPENED on first open', () => {
    const mockAutoClose: MockProxy<AutoClose> = mock<AutoClose>();
    mockGuardFix(mockAutoClose, 'close', Symbol.dispose);

    // const mockOpen = mock<{ open: () => AutoClose }>();
    const mockOpen: MockProxy<Open> = mock<Open>();
    mockGuardFix(mockOpen, 'open');
  ...
  //     const autoClose2 = idempotent.open();
  //     strictEqual(idempotent.getState(), 'OPENED', 'Second cycle: OPENED');
  //     autoClose2.close();
  ...
  //   it('should require contracts in config', () => {
  //     const mockOpen = mock<{ open: () => AutoClose }>();
  ...
  //     const mockAutoClose = mock<AutoClose>();
  //     const mockAutoOpen = mock<{ autoOpen: () => AutoClose }>();
  //     mockAutoOpen.autoOpen.mockReturnValue(mockAutoClose);
  ...
  //     const mockAutoClose1 = mock<AutoClose>();
  //     const mockAutoClose2 = mock<AutoClose>();
  //     const mockOpen = mock<{ open: () => AutoClose }>();
  //     mockOpen.open.mockReturnValue(mockAutoClose1);
